...
This repository contains some components that can be built and released
independently:

- [`mod_loader`](src/mod_loader): a generic Windows mod loader and its mod ABI.
- [`dynamic_fps_limit`](src/dynamic_fps_limit): a generic Streamline
  Reflx/RTSS dynamic FPS base on DLSS-G state mod using that ABI.
- [`reflex_fps_limit`](src/reflex_fps_limit): a generic NVIDIA Streamline
...
one artifact per component. See each component's README for local build
commands.
